...
​	②程序的入口，包括段地址和偏移地址。段地址的协定方法是，在用户程序header段里 保存 section.code_1.start，这个数据只是code_1相对于程序开始的偏移地址，因此我们得加上实际程序开头的物理地址，然后我们得到了code_1段开头的物理地址，再/16就得到了一个合适的段地址。而偏移地址，由于code_1段里有vstart=0 所以 里面的start标号永远是正确的相对于code_1段段地址的偏移地址，所以我们也把start标号保存在header的指定位置
